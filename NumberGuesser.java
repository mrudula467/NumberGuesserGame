...
public class NumberGuesser {
	...
	/**
	 * the method that tries to 'guess' the number uses binary search algorithm
	 * Recursion is not used as it adds a stack-frame for each guess Uses
	 * iteration instead Time Complexity O(log n) where n is the number of
	 * integers between maximum and minimum
	 * 
	 * @param lower
	 * @param higher
	 * @throws Exception
	...
	public void guessNumber(int lower, int higher) throws IOException {
		...
		while (true) {
			...
			numAttempts++;
			// Binary search
			System.out.println("Is the number - " + guess + " ? (Please answer with HIGHER, LOWER, or YES):");
	...
	public static void main(final String[] args) {
		...
		System.out.println("Choose a random number (integer) between " + NumberGuesserConstants.MIN_NUMBER + " "
				+ NumberGuesserConstants.MAX_NUMBER);

		try {

			numGuesser.guessNumber(NumberGuesserConstants.MIN_NUMBER, NumberGuesserConstants.MAX_NUMBER);
		} catch (IOException e) {
